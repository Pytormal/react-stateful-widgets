...
export default function Programmers() {
  ...
  const [programmers, setProgrammer] = useState(listOfAwesome);
  const [id, setId] = useState();
  

  const getNameOfFeatured = (programmers, id) => {
    // Leave this for last!
    ...
    let programmersName = programmers[id - 1].name
    ...
    <div className='widget-programmers container'>
      ...
      </div>
      {
        ...
        // Replace the hard-coded false with the correct variable.
         id
          ? <div style={style}>🎉 Let&apos;s celebrate {getNameOfFeatured(programmers, id)}! 🥳</div>
